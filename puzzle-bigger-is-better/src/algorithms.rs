...
impl<E: PairingEngine> ILV<E> {
    ...
    pub fn open(ck: &CommitmentKey<E>, a: &[E::Fr], b: &[E::Fr]) -> Proof<E> {
        ...
        let inner_product_commit = ck.powers_of_beta_g_first[dim+1].mul(inner_product);

        Proof(proof.into())
    }

    pub fn malicious_open(ck: &CommitmentKey<E>, a: &[E::Fr], b: &[E::Fr], claimed_inner_product: &E::Fr) -> Proof<E> {
        let dim = a.len();
        ...
        let inner_product_commit = ck.powers_of_beta_g_first[dim+1].mul(inner_product);
        let claimed_inner_product_commit = ck.powers_of_beta_g_first[dim+1].mul(*claimed_inner_product);
        ...
        let proof = first_part + second_part + inner_product_commit - claimed_inner_product_commit;
        Proof(proof.into())
    }
